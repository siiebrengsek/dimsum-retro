...
export const FinancialManagement = () => {
  ...
  const stockTotals = useMemo(() => {
    let list = stockSearch.trim()
      ? pendingStock.filter(i => i.name.toLowerCase().includes(stockSearch.toLowerCase()))
      : pendingStock;
    ...
    const totals = sorted.reduce(
      (acc, item) => ({ stok_kemarin: acc.stok_kemarin + item.stok_kemarin, perubahan: acc.perubahan + item.perubahan, sisa_hari_ini: acc.sisa_hari_ini + item.sisa_hari_ini }),
    ...
    if (existing) {
      ...
      if (Array.isArray(r.setoran_items) && r.setoran_items.length > 0) {
        setSetoranItems(r.setoran_items.map((s: any) => ({
          ...
          totalTerjual: String(s.totalTerjual ?? '0'), setoranOnline: String(s.setoranOnline ?? '0'),
        })));
      } else {
        setSetoranItems([{ id: newId(), name: 'Setoran Cash', totalTerjual: String(r.total_terjual_dimsum || 0), setoranOnline: String(Number(r.setoran_online) || 0) }]);
      }
    } else {
  ...
  const loadStockData = useCallback(async (date: string, outlet: string) => {
    ...
    if (existing) {
      ...
      const invNames = await getInventoryNames();
      setInventoryRef(invNames);
      setPendingStock(invNames.map(name => ({ name, stok_kemarin: prevMap.get(name) || 0, perubahan: 0, sisa_hari_ini: prevMap.get(name) || 0 })));
    }
  }, [getInventoryNames]);
  ...
  const toggleStockSort = (key: 'name' | 'stok_kemarin') => {
    setStockSortKey(prev => {
      if (prev === key) { setStockSortDir(d => (d === 'asc' ? 'desc' : 'asc')); return prev; }
      setStockSortDir('asc');
      return key;
    });
  };
...
Actually, let me rewrite this more carefully. I need to fix the filteredStock reference.<｜end▁of▁thinking｜>

<｜｜DSML｜｜tool_calls>
<｜｜DSML｜｜invoke name="write">
<｜｜DSML｜｜parameter name="content" string="true">import { useState, useEffect, useMemo, useCallback } from 'react';
import { supabase } from '../../lib/supabase';
import { getTodayDate } from '../../utils/dateUtils';
import { FaMoneyBillWave, FaPlus, FaTrash, FaSave, FaSyncAlt, FaBoxes, FaInfoCircle, FaSearch } from 'react-icons/fa';
                      ...
                      {setoranItems.map(item => {
                        ...
                        return (
                          <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                            ...
                            <td className="px-3 py-2 text-right">
                              {setoranItems.length > 1 && (
                                <button onClick={() => removeSetoranRow(item.id)} className="text-gray-400 hover:text-red-500 transition-colors p-1"><FaTrash size={12} /></button>
                              )}
                            </td>
                          </tr>
                        );
